...
class Helix:
    ...
    def create_helix_sections(self) -> List[int]:
        ...
        r1 = self.config.r[0]
        r2 = self.config.r[-1]
        h = self.config.modelaxi.h
        cut = self.config.cutwidth
        sens = 1 if self.config.odd else -1
        ...
        gmsh.model.occ.rotate([(2, s)], 0, 0, 0, 1, 0, 0, math.pi / 2.0)
        # gmsh.model.occ.synchronize()

        # TODO variable helical cut: pitch and turns form self.config.modelaxi
        # first compact modelaxi
        new_turns, new_pitchs = self.config.modelaxi.compact()
        sections = []
        theta = 0.0
        z = -h + self.z_offset
    ...
    def add_edm_start_hole(self, hcut) -> Tuple[List, Optional[Tuple]]:
        ...
        r2 = self.config.r[-1]
        h = self.config.modelaxi.h
        d = self.start_hole_diameter
        sens = 1 if self.config.odd else -1
        cut = self.config.cutwidth

        _z = -sens * h + sens * (d / 2 - cut / 2.0)
        _z += self.z_offset
        _r0 = r1 - self.eps
        ...
        outDimTags, _ = gmsh.model.occ.fragment(
            hcut, [(3, start)], removeObject=True, removeTool=True
        )
    ...
    def generate(self, ignore_ids: List[int] = []) -> List[int]:
        ...
        sections = self.create_helix_sections()
        ...
        dz = abs(z2 - z1)
        cint = gmsh.model.occ.addCylinder(0, 0, z1 + self.z_offset, 0, 0, dz, r1)
        cext = gmsh.model.occ.addCylinder(0, 0, z1 + self.z_offset, 0, 0, dz, r2)
        outDimTags, _ = gmsh.model.occ.cut(
            [(3, cext)], [(3, cint)], removeObject=True, removeTool=True
        )
        cyl = outDimTags[0]
        gmsh.model.occ.synchronize()
        ...
        if self.config.dble:
            ...
            hcut_rotated = []
            for dimtag in hcut:
                ov = gmsh.model.occ.copy([dimtag])
                gmsh.model.occ.rotate(ov, 0, 0, 0, 0, 0, 1, math.pi)
                gmsh.model.occ.synchronize()
                hcut_rotated.append(ov[0])

            hcut.extend(hcut_rotated)
            gmsh.model.occ.synchronize()
    ...
    def create_boundary_groups(
        self, volume_ids: List[int], bcs: Dict, bcs_type: Dict, interface: List[int], prefix=""
    ) -> Dict:
